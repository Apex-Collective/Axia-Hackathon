    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 md:px-6 pb-16">
      ...
      <div className="mb-8">
        ...
        <div className="bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 min-h-[300px] flex flex-col items-center justify-center p-8">
          ...
          <div className="w-24 h-24 bg-blue-100  hover:bg-blue-300 transition-colors rounded-full flex items-center justify-center mb-6 relative">
          ...
          <button className="bg-[#0F1115] text-white text-sm font-medium px-12 py-3 rounded-lg hover:bg-black/70 transition-colors shadow-sm cursor-pointer">
            Start Applying
          </button>
        </div>
      </div>
...
function JobCard({ job }: { job: Job }) {
  return (
    <div className="bg-white rounded-xl border-2 border-gray-500 p-6 flex flex-col gap-3 hover:shadow-lg transition-shadow duration-200 z-05">
      {/* Top Row: Logo & Bookmark */}
      <div className="flex justify-between items-start mb-4">
        {/* Placeholder Logo (Replace with <img> for real logos) */}
        <div
          className={`w-10 h-10 ${job.logoColor} text-white rounded-lg flex items-center justify-center font-bold text-lg`}
        >
          {job.logoLetter}
        </div>
        <button className="text-gray-300 hover:text-gray-600 cursor-pointer">
          <Bookmark
            size={20}
            className="fill-current text-gray-300 stroke-current"
          />
        </button>
      </div>
      ...
      <div className="mb-2 flex items-center gap-2">
        ...
        {job.isNew && (
          <span className="bg-purple-100 text-purple-600 text-[10px] font-bold px-2 py-1 rounded-sm uppercase tracking-wide">
            New post
          </span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {job.tags.map((tag) => (
            <span
              key={tag}
              className="bg-[#EFF2F6] text-gray-600 font-medium px-3 py-1.5 rounded-md"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="flex items-end justify-between mt-10">
        ...
        <button className="bg-[#0F1115] text-white text-sm font-medium px-5 py-2 rounded-lg hover:bg-black transition-colors cursor-pointer">
          Apply Now
        </button>
      </div>
    </div>
